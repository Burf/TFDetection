...
from tfdet.core.util import (pipeline, convert_to_numpy as convert_to_numpy_eager, convert_to_pickle as convert_to_pickle_eager, py_func, dict_function,
...
def convert_to_pickle(*args):
    if args and isinstance(args[0], dict):
        out = tf.py_function(convert_to_pickle_eager, tuple(args[0].values()), Tout = [tf.string] * len(args[0]))
        out = {k:v for k, v in zip(args[0].keys(), out)}
    else:
        out = tf.py_function(convert_to_pickle_eager, args, Tout = [tf.string] * len(args))
            ...
            if dtype is None:
                if isinstance(x_true, tf.data.Dataset):
                    ...
                    if isinstance(sample_args, dict):
                        sample_args = convert_to_numpy_eager(sample_args)
                        sample_result = function(**sample_args, **kwargs)
                    else:
                        sample_args = convert_to_numpy_eager([v for v in (sample_args if 0 < np.ndim(sample_args) else [sample_args])])
                        sample_result = function(*sample_args, **kwargs)
                else:
