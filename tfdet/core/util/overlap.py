...
def overlap_bbox(bbox_true, bbox_pred, mode = "normal"):
    ...
    """
    if mode not in ("normal", "general", "complete", "distance"):
        raise ValueError("unknown mode '{0}'".format(mode))
    
    true_count = tf.shape(bbox_true)[0]
    pred_count = tf.shape(bbox_pred)[0]
        
    bbox_true = tf.tile(bbox_true, [pred_count, 1])
    bbox_pred = tf.reshape(tf.tile(tf.expand_dims(bbox_pred, 0), [1, 1, true_count]), [-1, 4])
    ...
    overlaps = tf.reshape(iou, [pred_count, true_count])
    return overlaps
...
def overlap_point(bbox_true, points, regress_range):
    ...
    bbox_true = tf.tile(bbox_true, [pred_count, 1])
    points = tf.reshape(tf.tile(tf.expand_dims(points, 0), [1, 1, true_count]), [-1, 2])
    regress_range = tf.reshape(tf.tile(tf.expand_dims(regress_range, 0), [1, 1, true_count]), [-1, 2])
    
    x1, y1, x2, y2 = tf.split(bbox_true, 4, axis = -1)
    px, py = tf.split(points, 2, axis = -1)
    area = tf.reshape((x2 - x1) * (y2 - y1), [pred_count, true_count])
    offset = tf.concat([px - x1, py - y1, x2 - px, y2 - py], axis = -1) #left, top, right, bottom
    offset = tf.reshape(offset, [pred_count, true_count, 4])
    regress_range = tf.reshape(regress_range, [pred_count, true_count, 2])
    ...
    min_flag = tf.equal(area, tf.reduce_min(pad_area, axis = -1, keepdims = True))
