...
def unet(x, n_class = 35, n_feature = 64, n_depth = 5, method = "bilinear", logits_activation = tf.keras.activations.sigmoid, normalize = tf.keras.layers.BatchNormalization, activation = tf.keras.activations.relu):
    if isinstance(x, list):
        ...
        for index in range(n_depth):
            ...
            out = tf.keras.layers.Conv2D(_n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "downsample{0}_conv1".format(index + 1))(out)
            ...
            out = tf.keras.layers.Conv2D(_n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "downsample{0}_conv2".format(index + 1))(out)
            if normalize is not None:
    ...
    for index in range(n_depth - 1):
        ...
        out = tf.keras.layers.Conv2D(_n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "upsample{0}_pre_conv".format(index + 1))(out)
        if normalize is not None:
        ...
        out = tf.keras.layers.Conv2D(_n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "upsample{0}_conv1".format(index + 1))(out)
        if normalize is not None:
        ...
        out = tf.keras.layers.Conv2D(_n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "upsample{0}_conv2".format(index + 1))(out)
        if normalize is not None:
    ...
    out = tf.keras.layers.Conv2D(n_class, 1, padding = "same", use_bias = True, activation = logits_activation, kernel_initializer = "he_normal", name = "logits")(out)
    return out

def unet_2plus(feature, n_class = 35, n_feature = 256, deep_supervision = False, method = "bilinear", logits_activation = tf.keras.activations.sigmoid, normalize = tf.keras.layers.BatchNormalization, activation = tf.keras.activations.relu):
    ...
    for i, x in zip(range(len(feature) - 1, -1, -1), reversed(feature)):
        ...
        for j, o in enumerate(out):
            ...
            o = tf.keras.layers.Conv2D(n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "{0}_{1}_post_conv".format(i, j + 1))(o)
            if normalize is not None:
    ...
    if deep_supervision:
        ...
        out = out[-1:]
    out = [tf.keras.layers.Conv2D(n_class, 3, padding = "same", activation = logits_activation, kernel_initializer = "he_normal", name = "logits{0}".format(i) if 1 < len(out) else "logits")(o) for i, o in enumerate(out)]
    if len(out) == 1:
        out = out[0]
    return out