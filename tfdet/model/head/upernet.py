...
def upernet(feature, n_class = 35, n_feature = 512, pool_scale = [1, 2, 3, 6], method = "bilinear", logits_activation = tf.keras.activations.sigmoid, normalize = tf.keras.layers.BatchNormalization, activation = tf.keras.activations.relu):
    ...
    psp_feature = PoolingPyramidModule(pool_scale, n_feature, method = method, normalize = normalize, activation = activation, name = "psp_feature_extract")(feature[-1])
    psp_feature = tf.keras.layers.Concatenate(axis = -1, name = "psp_feature")([feature[-1]] + psp_feature)
    psp_feature = tf.keras.layers.Conv2D(n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "psp_feature_conv")(psp_feature)
    if normalize is not None:
    ...
    for index in range(len(out) - 1): #without psp feature
        o = tf.keras.layers.Conv2D(n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "feature{0}_post_conv".format(index + 1))(out[index])
        if normalize is not None:
    ...
    out = tf.keras.layers.Conv2D(n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "head_conv")(out)
    if normalize is not None:
