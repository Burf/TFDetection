...
class AtrousSpatialPyramidPooling(tf.keras.layers.Layer):
    def __init__(self, rate = [6, 12, 18], n_feature = 256, method = "bilinear", normalize = tf.keras.layers.BatchNormalization, activation = tf.keras.activations.relu, **kwargs):
    ...
    def build(self, input_shape):
        ...
        for kernel_size, dilation_rate in zip([1] + [3] * len(self.rate), [1] + list(self.rate)):
            layer = []
            layer.append(tf.keras.layers.Conv2D(self.n_feature, kernel_size, dilation_rate = dilation_rate, padding = "same", use_bias = self.normalize is None, kernel_initializer = "he_normal"))
...
def deeplab_v3(x, n_class = 35, rate = [6, 12, 18], n_feature = 256, n_low_feature = 48, method = "bilinear", logits_activation = tf.keras.activations.sigmoid, normalize = tf.keras.layers.BatchNormalization, activation = tf.keras.activations.relu):
    if not isinstance(x, list):
        x = [x]
    low_features, feature = x[:-1], x[-1]
    
    #aspp
    feature = AtrousSpatialPyramidPooling(rate, n_feature, method, normalize = normalize, activation = activation, name = "aspp_feature" if len(low_features) == 0 else "aspp_feature1")(feature)

    #decoder - deeplab v3 plus
    for index, low_feature in enumerate(low_features[::-1]):
        low_feature = tf.keras.layers.Conv2D(n_low_feature * (2 ** (len(low_features) - (index + 1))), 1, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "low_feature{0}_conv".format(len(low_features) - index))(low_feature)
        ...
        feature = tf.keras.layers.Conv2D(n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "feature{0}_conv".format(index + 2))(feature)
        if normalize is not None:
    ...
    #head
    out = tf.keras.layers.Conv2D(n_feature, 3, padding = "same", use_bias = normalize is None, kernel_initializer = "he_normal", name = "head_conv")(feature)
    if normalize is not None:
