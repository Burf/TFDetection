...
def bifpn_lite(n_feature = 88, use_bias = True, weighted_add = False, method = "nearest", normalize = tf.keras.layers.BatchNormalization, activation = tf.nn.relu6, mode = "bifpn", **kwargs):
...
def effdet(x, n_class = 21, image_shape = [1024, 1024], n_feature = 224, n_depth = 4, sub_sampling = 2,
           ...
           cls_normalize = tf.keras.layers.BatchNormalization, cls_activation = tf.nn.swish, 
           box_normalize = tf.keras.layers.BatchNormalization, box_activation = tf.nn.swish):
    out = retinanet(x, n_class = n_class, image_shape = image_shape, n_feature = n_feature, n_depth = n_depth, sub_sampling = sub_sampling,
                    ...
                    cls_normalize = cls_normalize, cls_activation = cls_activation, 
                    box_normalize = box_normalize, box_activation = box_activation)
    return out
...
def effdet_lite(x, n_class = 21, image_shape = [384, 384], n_feature = 88, n_depth = 3, sub_sampling = 2,
                ...
                cls_normalize = tf.keras.layers.BatchNormalization, cls_activation = tf.nn.relu6, 
                box_normalize = tf.keras.layers.BatchNormalization, box_activation = tf.nn.relu6):
    out = retinanet(x, n_class = n_class, image_shape = image_shape, n_feature = n_feature, n_depth = n_depth, sub_sampling = sub_sampling,
                    scale = scale, ratio = ratio, auto_scale = auto_scale,
                    sub_n_feature = sub_n_feature, sub_normalize = sub_normalize, 
                    neck = neck, neck_n_depth = neck_n_depth,
