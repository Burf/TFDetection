...
def train_model(input, logits, regress, anchors,
                ...
                regularize = True, weight_decay = 1e-4, focal = True, alpha = .25, gamma = 1.5, sigma = 3, class_weight = None, missing_value = 0.):
    ...
    target_y_true, target_bbox_true, target_y_pred, target_bbox_pred = tf.keras.layers.Lambda(lambda args: map_fn(anchor_target, *args, dtype = (y_true.dtype, bbox_true.dtype, logits.dtype, regress.dtype), batch_size = batch_size, 
                                                                                                                  assign = assign, sampling_count = sampling_count, positive_ratio = positive_ratio, mean = mean,std = std), name = "anchor_target")([y_true, bbox_true, logits, regress, tile_anchors])
