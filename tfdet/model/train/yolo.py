...
from ..loss.yolo import score_loss, logits_accuracy, logits_loss, regress_loss
...
def train_model(input, score, logits, regress, anchors,
                ...
                batch_size = 1, mean = [0., 0., 0., 0.], std = [0.1, 0.1, 0.2, 0.2], clip_ratio = 16 / 1000, regularize = True, weight_decay = 1e-4, mode = "general", focal = True, alpha = .25, gamma = 1.5, class_weight = None, missing_value = 0.):
    ...
    score_true, logits_true, _bbox_true, score_pred, logits_pred, bbox_pred = tf.keras.layers.Lambda(lambda args: map_fn(yolo_target, *args, dtype = (tf.int32, y_true.dtype, bbox_true.dtype, score.dtype, logits.dtype, regress.dtype), batch_size = batch_size, 
                                                                                                                         sampling_count = sampling_count, positive_ratio = positive_ratio, positive_threshold = positive_threshold, negative_threshold = negative_threshold, mean = mean, std = std, clip_ratio = clip_ratio), name = "yolo_target")([y_true, bbox_true, score, logits, regress, anchors])
