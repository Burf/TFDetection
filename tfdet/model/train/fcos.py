...
def train_model(input, logits, regress, points, centerness = None,
                sampling_count = 256, positive_ratio = 0.5,
    ...
    if centerness is not None:
        target_y_true, target_bbox_true, target_centerness_true, target_y_pred, target_bbox_pred, target_centerness_pred = tf.keras.layers.Lambda(lambda args: map_fn(point_target, *args, dtype = (y_true.dtype, bbox_true.dtype, bbox_true.dtype, logits.dtype, regress.dtype, centerness.dtype), batch_size = batch_size, sampling_count = sampling_count, positive_ratio = positive_ratio), name = "point_target")([y_true, bbox_true, logits, regress, points, regress_range, centerness])
    else:
        target_y_true, target_bbox_true, target_y_pred, target_bbox_pred = tf.keras.layers.Lambda(lambda args: map_fn(point_target, *args, dtype = (y_true.dtype, bbox_true.dtype, logits.dtype, regress.dtype), batch_size = batch_size, sampling_count = sampling_count, positive_ratio = positive_ratio), name = "point_target")([y_true, bbox_true, logits, regress, points, regress_range])
