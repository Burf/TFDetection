...
def filter_detection(score, logit, regress, anchors, proposal_count = 100, iou_threshold = 0.3, score_threshold = 0.7, soft_nms = True, mean = [0., 0., 0., 0.], std = [0.1, 0.1, 0.2, 0.2], clip_ratio = 16 / 1000):
    ...
    score = tf.gather(score, valid_indices)
...
class FilterDetection(tf.keras.layers.Layer):
    ...
    def call(self, inputs, proposal_count = 100, iou_threshold = 0.3, score_threshold = 0.7, soft_nms = True, batch_size = 1, mean = [0., 0., 0., 0.], std = [0.1, 0.1, 0.2, 0.2], clip_ratio = 16 / 1000):
