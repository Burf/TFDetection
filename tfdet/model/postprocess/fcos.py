...
class FilterDetection(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        out = map_fn(multiclass_nms, logits, regress, points, dtype = (logits.dtype, regress.dtype), batch_size = self.batch_size,
                     proposal_count = self.proposal_count, soft_nms = self.soft_nms, iou_threshold = self.iou_threshold, score_threshold = self.score_threshold, performance_count = self.performance_count, coder_func = offset2bbox)
        return out
