...
def resnet18(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    ...
    layers = ["stem_act", "stage1_block2_out", "stage2_block2_out", "stage3_block2_out", "stage4_block2_out"]
    if -1 < frozen_stages:
...
def resnet34(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    imagenet > normalize(x, rmean = [123.675, 116.28, 103.53], std = [58.395, 57.12, 57.375])
    """
    ...
    layers = ["stem_act", "stage1_block3_out", "stage2_block4_out", "stage3_block6_out", "stage4_block3_out"]
    if -1 < frozen_stages:
        for l in model.layers:
            l.trainable = False
            if l.name == layers[frozen_stages]:
...
def resnet50(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet_v2", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    imagenet > normalize(x, rmean = [123.675, 116.28, 103.53], std = [58.395, 57.12, 57.375])
    """
    ...
    if weights == "imagenet":
        ...
        load_weight(model, resnet_imagenet_v2_urls["resnet50"])
    elif weights is not None:
        model.load_weights(weights)
...
def resnet101(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet_v2", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    imagenet > normalize(x, rmean = [123.675, 116.28, 103.53], std = [58.395, 57.12, 57.375])
    """
    ...
    if weights == "imagenet":
        ...
        load_weight(model, resnet_imagenet_v2_urls["resnet101"])
    elif weights is not None:
        model.load_weights(weights)
        
    layers = ["stem_act", "stage1_block3_out", "stage2_block4_out", "stage3_block23_out", "stage4_block3_out"]
    if -1 < frozen_stages:
        for l in model.layers:
            l.trainable = False
            if l.name == layers[frozen_stages]:
...
def resnet152(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet_v2", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    imagenet > normalize(x, rmean = [123.675, 116.28, 103.53], std = [58.395, 57.12, 57.375])
    """
    ...
    if weights == "imagenet":
        ...
        load_weight(model, resnet_imagenet_v2_urls["resnet152"])
    elif weights is not None:
        model.load_weights(weights)
        
    layers = ["stem_act", "stage1_block3_out", "stage2_block8_out", "stage3_block36_out", "stage4_block3_out"]
    if -1 < frozen_stages:
        for l in model.layers:
            l.trainable = False
            if l.name == layers[frozen_stages]:
...
def resnext50_32x4d(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet_v2", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    imagenet > normalize(x, rmean = [123.675, 116.28, 103.53], std = [58.395, 57.12, 57.375])
    """
    ...
    if weights == "imagenet":
        ...
        load_weight(model, resnet_imagenet_v2_urls["resnext50_32x4d"])
    elif weights is not None:
        model.load_weights(weights)
...
def resnext101_32x8d(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet_v2", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    imagenet > normalize(x, rmean = [123.675, 116.28, 103.53], std = [58.395, 57.12, 57.375])
    """
    ...
    if weights == "imagenet":
        ...
        load_weight(model, resnet_imagenet_v2_urls["resnext101_32x8d"])
    elif weights is not None:
        model.load_weights(weights)
...
def resnext101_64x4d(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    imagenet > normalize(x, rmean = [123.675, 116.28, 103.53], std = [58.395, 57.12, 57.375])
    """
    out = resnet(x, [3, 4, 23, 3], res_bottleneck_block, include_top = False, group_size = 64, base_width = 4, normalize = normalize, activation = activation)
    model = tf.keras.Model(x, out)

    if weights == "imagenet":
        load_weight(model, resnet_urls["resnext101_64x4d"])
    elif weights is not None:
        model.load_weights(weights)
...
def wide_resnet50_2(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet_v2", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    imagenet > normalize(x, rmean = [123.675, 116.28, 103.53], std = [58.395, 57.12, 57.375])
    """
    ...
    if weights == "imagenet":
        ...
        load_weight(model, resnet_imagenet_v2_urls["wide_resnet50_2"])
    elif weights is not None:
        model.load_weights(weights)
...
def wide_resnet101_2(x, normalize = normalize, activation = tf.keras.activations.relu, weights = "imagenet_v2", indices = [0, 1, 2, 3], frozen_stages = -1):
    """
    imagenet > normalize(x, rmean = [123.675, 116.28, 103.53], std = [58.395, 57.12, 57.375])
    """
    ...
    if weights == "imagenet":
        ...
        load_weight(model, resnet_imagenet_v2_urls["wide_resnet101_2"])
    elif weights is not None:
        model.load_weights(weights)
